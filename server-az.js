...
const TWITCH_CFG_FILE = path.join(__dirname, 'twitch-server.json');
...
// ── DATA RAPPORTS ──
let rapportsData = [];
if (fs.existsSync(RAPPORTS_FILE)) {
try { rapportsData = JSON.parse(fs.readFileSync(RAPPORTS_FILE, 'utf8')); } catch(e) {}
}
function saveRapports() {
try { fs.writeFileSync(RAPPORTS_FILE, JSON.stringify(rapportsData, null, 2)); } catch(e) {}
}

// ── CONFIG TWITCH SERVEUR ──
let twitchCfg = { clientId:'', clientSecret:'', userId:'', username:'' };
if (fs.existsSync(TWITCH_CFG_FILE)) {
try { twitchCfg = { ...twitchCfg, ...JSON.parse(fs.readFileSync(TWITCH_CFG_FILE, 'utf8')) }; } catch(e) {}
}
function saveTwitchCfg() { fs.writeFileSync(TWITCH_CFG_FILE, JSON.stringify(twitchCfg, null, 2)); }
...
twitchTokenExpiry = Date.now() + (json.expires_in - 300) * 1000;
...
return new Promise(resolve => {
const req = https.request({
hostname: 'api.twitch.tv', path: apiPath, method: 'GET',
...
let streamState = {
live: false, streamId: null, startedAt: null,
title: '', gameName: '', followersAtStart: 0,
viewerSnapshots: [], chatMessages: 0, topChatters: {}
};
...
streamState = {
live: true, streamId: s.id, startedAt: s.started_at,
title: s.title, gameName: s.game_name,
followersAtStart: follows?.total || 0,
viewerSnapshots: [{ ts: Date.now(), count: s.viewer_count }],
chatMessages: 0, topChatters: {}
};
...
streamState.title = s.title;
streamState.gameName = s.game_name;
} else if (!isLive && streamState.live) {
...
const topChatters = Object.entries(streamState.topChatters || {}).sort((a,b) => b[1]-a[1]).slice(0,5).map(([u]) => u);
const rapport = {
id: Date.now().toString(36),
startedAt: streamState.startedAt || new Date().toISOString(),
endedAt: new Date().toISOString(),
title: streamState.title || 'Stream',
gameName: streamState.gameName || '',
durationMs, avgViewers, peakViewers, newFollowers,
chatMessages: streamState.chatMessages || 0,
topChatters
};
...
broadcast({ type: 'rapport_ready', rapport });
}
...
if (pathname === '/api/rapports' && req.method === 'GET') {
res.writeHead(200, { 'Content-Type': 'application/json' });
return res.end(JSON.stringify(rapportsData));
}

if (pathname === '/api/rapports' && req.method === 'DELETE') {
rapportsData = []; saveRapports();
res.writeHead(200, { 'Content-Type': 'application/json' });
return res.end(JSON.stringify({ ok: true }));
}
...
const id = rapportDelMatch[1];
rapportsData = rapportsData.filter(r => r.id !== id); saveRapports();
res.writeHead(200, { 'Content-Type': 'application/json' });
return res.end(JSON.stringify({ ok: true }));
}
...
const test = {
id: Date.now().toString(36),
startedAt: new Date(Date.now() - (2 + Math.random() * 3) * 3600000).toISOString(),
endedAt: new Date().toISOString(),
title: titles[i], gameName: games[i],
durationMs: Math.floor((2 + Math.random() * 4) * 3600000),
avgViewers: Math.floor(8 + Math.random() * 40),
peakViewers: Math.floor(20 + Math.random() * 80),
newFollowers: Math.floor(Math.random() * 15),
chatMessages: Math.floor(50 + Math.random() * 500),
topChatters: ['darkwolf42','lazer_hbr','trainspotter99','vyzfan','raideur_xl'].slice(0, 3 + Math.floor(Math.random() * 3))
};
rapportsData.unshift(test);
if (rapportsData.length > 50) rapportsData = rapportsData.slice(0, 50);
saveRapports();
res.writeHead(200, { 'Content-Type': 'application/json' });
return res.end(JSON.stringify({ ok: true, rapport: test }));
}
